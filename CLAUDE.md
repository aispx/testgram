...
## Auto-Delete Messages Implementation

### Background Service

**File:** `source/src/MyTelegram.Messenger/Services/HistoryTTL/MessageAutoDeleteService.cs`

**Purpose:** Automatically deletes expired messages based on dialog TTL settings.

**How it works:**
1. Runs every 5 minutes
2. Queries dialogs with `TtlPeriod > 0`
3. For each dialog, finds messages older than TTL
4. Soft-deletes expired messages (sets `Deleted = true`)

**Registration:** Added to `Program.cs` as hosted service:
```csharp
services.AddHostedService<MyTelegram.Messenger.Services.HistoryTTL.MessageAutoDeleteService>();
```

### TTL Flow

**1. User sets default TTL:**
```
Client → messages.setDefaultHistoryTTL(period=86400)
       → SetDefaultHistoryTTLHandler
       → Updates eventflow-userreadmodel.DefaultHistoryTTL
```

**2. User sets chat-specific TTL:**
```
Client → messages.setHistoryTTL(peer, period=604800)
       → SetHistoryTTLHandler
       → Updates dialogs.TtlPeriod
       → Sends TMessageActionSetMessagesTTL service message
```

**3. Background service deletes expired messages:**
```
Every 5 minutes:
  → Query dialogs where TtlPeriod > 0
  → For each dialog:
      currentTime - TtlPeriod = expirationTime
      Find messages where Date < expirationTime
      Set Deleted = true
```

### Testing Auto-Delete

**1. Set TTL for a chat:**
```bash
# In Telegram client:
# Settings → Privacy → Auto-Delete Messages → Set for specific chat
```

**2. Check dialog TTL in MongoDB:**
```javascript
docker-compose exec mongodb mongosh tg
db.dialogs.find({ TtlPeriod: { $gt: 0 } }).pretty()
```

**3. Check service logs:**
```bash
docker-compose logs -f messenger-command-server | grep -i "auto-delete\|ttl"
```

**4. Manually trigger deletion (for testing):**
```javascript
// Set very short TTL (60 seconds)
db.dialogs.updateOne(
  { _id: "userId_peerType_peerId" },
  { $set: { TtlPeriod: 60 } }
)

// Wait 5 minutes for background service to run
// Or restart server to trigger immediately
```

**5. Verify messages deleted:**
```javascript
db['eventflow-messagereadmodel'].find({
  OwnerPeerId: NumberLong("userId"),
  Deleted: true
}).count()
```
...
**Check interval:** 5 minutes (configurable in `MessageAutoDeleteService.cs`)
...
1. Check if TTL is set:
```javascript
db.dialogs.find({ TtlPeriod: { $gt: 0 } })
```

2. Check service is running:
```bash
docker-compose logs messenger-command-server | grep "Message Auto-Delete Service"
```

3. Check for errors:
```bash
docker-compose logs messenger-command-server | grep -i "error.*auto-delete"
```

4. Manually test deletion logic:
```javascript
var currentTime = Math.floor(Date.now() / 1000);
var ttlPeriod = 86400; // 1 day
