        ...
        ISagaHandles<MessageAggregate, MessageId, MessageForwardedEvent>
{
    ...
    private readonly IMessageEncryptionHelper _encryptionHelper;
    private readonly ForwardMessageState _state = new();

    public ForwardMessageSaga(ForwardMessageSagaId id, IEventStore eventStore, IIdGenerator idGenerator, IMessageEncryptionHelper encryptionHelper) : base(id, eventStore)
    {
    ...
    private void ForwardMessage(ForwardMessagesStartedEvent aggregateEvent)
    {
        ...
        var uniqueMessages = new HashSet<int>();
        var index = 0;
        foreach (var messageId in aggregateEvent.MessageIds)
        {
            if (index >= aggregateEvent.RandomIds.Count || !uniqueMessages.Add(messageId))
            {
                index++;
                continue;
            }
    ...
    private async Task<int> SendMessageToTargetPeerAsync(MessageForwardedEvent aggregateEvent)
    {
        ...
        // Decrypt if message is stored encrypted
        if (string.IsNullOrEmpty(message) && item.EncryptedData != null)
        {
            message = _encryptionHelper.Decrypt(item.OwnerPeer.PeerId, item.MessageId, item.EncryptedData.Value);
        }

        // Skip forwarding if message, media and encrypted data are all empty
        if (string.IsNullOrEmpty(message) && item.EncryptedData == null && item.Media == null)
        {
            return outMessageId;
        }
