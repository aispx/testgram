...
public class ChannelDomainEventHandler(
    ...
    : DomainEventHandlerBase(objectMessageSender,
        ...
        ISubscribeSynchronousTo<ChannelAggregate, ChannelId, MonoforumEnabledEvent>
{
    ...
    public async Task HandleAsync(IDomainEvent<ChannelAggregate, ChannelId, MonoforumEnabledEvent> domainEvent, CancellationToken cancellationToken)
    {
        var e = domainEvent.AggregateEvent;
        var updateChannel = new TUpdateChannel { ChannelId = e.ChannelId };
        var updates = new TUpdates { Updates = new TVector<IUpdate>(updateChannel), Users = [], Chats = [], Date = DateTime.UtcNow.ToTimestamp() };
        await SendRpcMessageToClientAsync(e.RequestInfo, updates);
        await NotifyUpdateChannelAsync(e.RequestInfo with { ReqMsgId = 0 }, e.ChannelId);
    }
}