...
/// </remarks>
internal sealed class GetEmojiGroupsHandler(IMongoDatabase mongoDatabase) : RpcResultObjectHandler<MyTelegram.Schema.Messages.RequestGetEmojiGroups, MyTelegram.Schema.Messages.IEmojiGroups>
{
    protected override async Task<MyTelegram.Schema.Messages.IEmojiGroups> HandleCoreAsync(IRequestInput input, MyTelegram.Schema.Messages.RequestGetEmojiGroups obj)
    {
        var collection = mongoDatabase.GetCollection<BsonDocument>("emoji_groups");
        var docs = await collection.Find(Builders<BsonDocument>.Filter.Or(
                Builders<BsonDocument>.Filter.Exists("For", false),
                Builders<BsonDocument>.Filter.Eq("For", "default")))
