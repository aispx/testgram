...
/// </remarks>
internal sealed class GetEmojiStatusGroupsHandler(IMongoDatabase mongoDatabase) : RpcResultObjectHandler<MyTelegram.Schema.Messages.RequestGetEmojiStatusGroups, MyTelegram.Schema.Messages.IEmojiGroups>
{
    protected override async Task<MyTelegram.Schema.Messages.IEmojiGroups> HandleCoreAsync(IRequestInput input, MyTelegram.Schema.Messages.RequestGetEmojiStatusGroups obj)
    {
        var collection = mongoDatabase.GetCollection<BsonDocument>("emoji_groups");
        var docs = await collection.Find(Builders<BsonDocument>.Filter.Eq("For", "status"))
