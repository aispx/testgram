...
/// </remarks>
internal sealed class GetEmojiStickerGroupsHandler(IMongoDatabase mongoDatabase) : RpcResultObjectHandler<MyTelegram.Schema.Messages.RequestGetEmojiStickerGroups, MyTelegram.Schema.Messages.IEmojiGroups>
{
    protected override async Task<MyTelegram.Schema.Messages.IEmojiGroups> HandleCoreAsync(IRequestInput input, MyTelegram.Schema.Messages.RequestGetEmojiStickerGroups obj)
    {
        var collection = mongoDatabase.GetCollection<BsonDocument>("emoji_groups");
        var docs = await collection.Find(Builders<BsonDocument>.Filter.Eq("For", "stickers"))
