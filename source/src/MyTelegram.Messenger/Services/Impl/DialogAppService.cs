    ...
    : BaseAppService, IDialogAppService, ITransientDependency
{
    public async Task<GetDialogOutput> GetDialogsAsync(GetDialogInput input)
    {
        ...
        // Add monoforums so clients can cache them
        var monoforumIds = channelReadModels
            .Where(c => c.LinkedMonoforumId.HasValue)
            .Select(c => c.LinkedMonoforumId!.Value)
            .Except(channelIds)
            .ToList();
        if (monoforumIds.Count > 0)
        {
            var monoforumReadModels = await channelAppService.GetListAsync(monoforumIds);
            channelReadModels = channelReadModels.Concat(monoforumReadModels).ToList();
        }
