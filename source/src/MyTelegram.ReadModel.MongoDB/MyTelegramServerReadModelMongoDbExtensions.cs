...
public static class MyTelegramServerReadModelMongoDbExtensions
{
    public static void RegisterMongoDbSerializer(this IServiceCollection services)
    {
        ...
        try { BsonSerializer.RegisterSerializer(objectSerializer); } catch (Exception) { }
        //BsonSerializer.RegisterSerializer(guidSerializer);

        var asm = baseType.Assembly;

        // Register a serialization provider so MongoDB can resolve discriminators
        ...
        foreach (var type in types)
        {
            ...
            if (!BsonClassMap.IsClassMapRegistered(type)) BsonClassMap.RegisterClassMap(cm);
        }
    }
