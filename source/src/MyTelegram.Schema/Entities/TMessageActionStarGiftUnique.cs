...
public sealed partial class TMessageActionStarGiftUnique : IMessageAction
{
    ...
    public void Serialize(IBufferWriter<byte> writer)
    {
        ...
        writer.Write(Flags);
        writer.Write(Gift);
        if (Flags.IsBitSet(3) && CanExportAt.HasValue) { writer.Write(CanExportAt.Value); }
        ...
        if (Flags.IsBitSet(12) && DropOriginalDetailsStars.HasValue) { writer.Write(DropOriginalDetailsStars.Value); }
        if (Flags.IsBitSet(15) && CanCraftAt.HasValue) { writer.Write(CanCraftAt.Value); }
    }
