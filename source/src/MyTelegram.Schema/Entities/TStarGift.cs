...
public sealed partial class TStarGift : ILayeredStarGift
{
    ...
    public void ComputeFlag()
    {
        ...
        if (Auction && AuctionSlug != null) { Flags = Flags.SetBit(11); }
        if (Auction && GiftsPerRound.HasValue) { Flags = Flags.SetBit(11); }
        if (Auction && AuctionStartDate.HasValue) { Flags = Flags.SetBit(11); }
        if (UpgradeVariants.HasValue) { Flags = Flags.SetBit(12); }
    ...
    public void Serialize(IBufferWriter<byte> writer)
    {
        ...
        writer.Write(Stars);
        if (Flags.IsBitSet(0) && AvailabilityRemains.HasValue) { writer.Write(AvailabilityRemains.Value); }
        if (Flags.IsBitSet(0) && AvailabilityTotal.HasValue) { writer.Write(AvailabilityTotal.Value); }
        if (Flags.IsBitSet(4) && AvailabilityResale.HasValue) { writer.Write(AvailabilityResale.Value); }
        writer.Write(ConvertStars);
        if (Flags.IsBitSet(1) && FirstSaleDate.HasValue) { writer.Write(FirstSaleDate.Value); }
        if (Flags.IsBitSet(1) && LastSaleDate.HasValue) { writer.Write(LastSaleDate.Value); }
        if (Flags.IsBitSet(3) && UpgradeStars.HasValue) { writer.Write(UpgradeStars.Value); }
        if (Flags.IsBitSet(4) && ResellMinStars.HasValue) { writer.Write(ResellMinStars.Value); }
        if (Flags.IsBitSet(5)) { writer.Write(Title); }
        if (Flags.IsBitSet(6)) { writer.Write(ReleasedBy); }
        if (Flags.IsBitSet(8) && PerUserTotal.HasValue) { writer.Write(PerUserTotal.Value); }
        if (Flags.IsBitSet(8) && PerUserRemains.HasValue) { writer.Write(PerUserRemains.Value); }
        if (Flags.IsBitSet(9) && LockedUntilDate.HasValue) { writer.Write(LockedUntilDate.Value); }
        if (Flags.IsBitSet(11)) { writer.Write(AuctionSlug ?? ""); }
        if (Flags.IsBitSet(11) && GiftsPerRound.HasValue) { writer.Write(GiftsPerRound.Value); }
        if (Flags.IsBitSet(11) && AuctionStartDate.HasValue) { writer.Write(AuctionStartDate.Value); }
        if (Flags.IsBitSet(12) && UpgradeVariants.HasValue) { writer.Write(UpgradeVariants.Value); }
