...
public sealed partial class TStarGiftAuctionUserState : IStarGiftAuctionUserState
{
    ...
    public void Serialize(IBufferWriter<byte> writer)
    {
        ...
        writer.Write(Flags);
        if (Flags.IsBitSet(0) && BidAmount.HasValue) { writer.Write(BidAmount.Value); }
        if (Flags.IsBitSet(0) && BidDate.HasValue) { writer.Write(BidDate.Value); }
        if (Flags.IsBitSet(0) && MinBidAmount.HasValue) { writer.Write(MinBidAmount.Value); }
        if (Flags.IsBitSet(0) && BidPeer != null) { writer.Write(BidPeer); }
